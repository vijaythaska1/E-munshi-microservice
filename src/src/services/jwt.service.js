...
let s = require('./user.service'),
  n = require('../config/config'),
  { Token: a } = require('../models'),
  { tokenTypes: o } = require('../config/tokens'),
  u = ({ userId: r, type: i, secret: s = n.jwt.secret }) => {
    let a = {
      sub: r,
      iat: t().unix(),
      type: i,
  ...
  d = async ({ token: e, userId: t, type: r, blacklisted: i = !1 }) =>
    await a.create({
      token: e,
      user: t,
      type: r,
