...
let i = require("./user.service"), n = require("../config/config"), { Token: o } = require("../models"), { tokenTypes: a } = require("../config/tokens"), u = ({ userId: r, type: s, secret: i = n.jwt.secret })=>{
    let o = {
        sub: r,
        iat: t().unix(),
        type: s
    ...
    return e.sign(o, i);
}, d = async ({ token: e, userId: t, type: r, blacklisted: s = !1 })=>await o.create({
        token: e,
        user: t,
        type: r,
        blacklisted: s
    }), l = async ({ token: t, type: r })=>{
    let s = e.verify(t, n.jwt.secret), i = await o.findOne({
        token: t,
        type: r,
        user: s.sub,
        blacklisted: !1
...
}, p = async (e)=>await o.findOneAndDelete({
        user: e.id
    });
module.exports = {
    generateToken: u,
