...
const routesFile = path.join(__dirname, "../../route-Json-File/auth.route.json");
...
routesData.forEach((route: {
    ...
    try {
        ...
        if (handler) {
            ...
            console.warn(`❌ Handler '${route.handler}' not found for route ${route.path}`);
        }
    } catch (routeError) {
